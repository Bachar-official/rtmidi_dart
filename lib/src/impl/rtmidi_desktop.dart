...
import '../device/midi_device_desktop.dart';

class RtMidiDesktop implements RtMidiImpl {
  final RtMidiFFI _bindings;

  RtMidiDesktop() : _bindings = RtMidiFFI(_loadLibrary());

  static DynamicLibrary _loadLibrary() {
    if (Platform.isAndroid) {
      throw UnsupportedError('RtMidi FFI not supported on Android');
    }
  ...
  @override
  Future<List<MidiDeviceDesktop>> get devices async {
    ...
    if (hasIn) {
      for (int i = 0; i < inCount; i++) {
        ...
        final info = grouped.putIfAbsent(
            normalized, () => MidiDeviceInfo(name: rawName));
        info.inputPort = i;
    ...
    return grouped.values
        ...
        .map((info) => MidiDeviceDesktop.fromInfo(info, _bindings))
        .toList();
  }
